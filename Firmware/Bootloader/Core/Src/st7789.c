 ...
 */
#define HOR_LEN 	2	//	Also mind the resolution of your screen!
...
void ST7789_WriteChar(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor)
{
	...
	uint8_t char_buf[font.width * font.height * 2];
	uint32_t i, j;
	uint32_t index = 0;
	...
	for (i = 0; i < font.height; i++) {
		...
		for (j = 0; j < font.width; j++) {
			...
			char_buf[index++] = pixel >> 8;      // High byte first
			char_buf[index++] = pixel & 0xFF;    // Low byte
		}
	}

	ST7789_SetAddressWindow(x, y, x + font.width - 1, y + font.height - 1);
	ST7789_WriteData(char_buf, sizeof(char_buf));
}
