...
/////////
bool Set_PD_CustomValues(uint8_t pdo_num, uint32_t voltage_mv, uint32_t current_ma);
...
#define STUSB4500_LOG(fmt, ...) do { \
    char log_buffer[128]; \
    snprintf(log_buffer, sizeof(log_buffer), fmt, ##__VA_ARGS__); \
    USB_SendMessage(log_buffer); \
} while (0)
...
typedef union {
    ...
    struct {
        ...
        uint32_t AugmentedPowerDO : 2;
    } apdo;
} usb_pd_pdo_t;

// Function Prototype
void stusb4500_read_and_print_src_pdos(void);
// External dependency for printing (from main.c)
extern void USB_SendMessage(const char *message);
