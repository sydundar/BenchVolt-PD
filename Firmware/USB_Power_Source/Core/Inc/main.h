...
/* Private defines -----------------------------------------------------------*/
#define EN_DC1_Pin GPIO_PIN_13
#define EN_DC1_GPIO_Port GPIOC
#define EN_DC2_Pin GPIO_PIN_2
#define EN_DC2_GPIO_Port GPIOB
...
void HardReset();
void Print_PDOs_ReceivedAtPowerUp();
void negotiate();
void dumpRegisters();
...
void DrawChannelFrame(uint8_t row, bool OnlyCircleFrame);
void Remote_Parse_Byte(uint8_t data);
...
/* USER CODE BEGIN PTD */
typedef struct {
    uint32_t Voltage; // mV (Örn: 5000)
    uint32_t Current; // mA (Örn: 3000)
