...
#define I2C1_SET_SDA    (I2C1_GPIO_Port->BSRR = SDA1_Pin)
#define I2C1_CLEAR_SDA  (I2C1_GPIO_Port->BSRR = (uint32_t)SDA1_Pin << 16U)
#define I2C1_SET_SCL    (I2C1_GPIO_Port->BSRR = SCL1_Pin)
#define I2C1_CLEAR_SCL  (I2C1_GPIO_Port->BSRR = (uint32_t)SCL1_Pin << 16U)
*/

#define I2C1_SET_SDA        (I2C1_GPIO_Port->BSRR = (uint32_t)SDA1_Pin)
#define I2C1_CLEAR_SDA      (I2C1_GPIO_Port->BSRR = (uint32_t)SDA1_Pin << 16U)

#define I2C1_SET_SCL        (I2C1_GPIO_Port->BSRR = (uint32_t)SCL1_Pin)
#define I2C1_CLEAR_SCL      (I2C1_GPIO_Port->BSRR = (uint32_t)SCL1_Pin << 16U)


#define I2C_DELAY delay_cycles(1)


void delay_Tick(uint32_t us) {
...
{
	return (I2C1_GPIO_Port->IDR & SDA1_Pin) ? 1 : 0;
}
...
{
    // 1. Cihaz adresi gönder (Yazma modunda)
    if (I2C_write_byte(address, true, false))
    {
        // 2. Register adresini gönder
        for (int i = 0; i < reg_size; i++) {
            if (!I2C_write_byte(reg[i], false, false)) break;
        }

        // 3. Re-start ve Cihaz adresi (Okuma modunda)
        // Adres zaten dışarıdan kaydırılmış (0x50) geliyor, okuma için bit 0 set edilir (0x51)
        if (I2C_write_byte(address | 0x01, true, false))
        {
            for (int j = 0; j < size; j++) {
                // KRİTİK: Son byte hariç ACK (0), son byte'ta NACK (1) gönderilir
                _Bool send_ack = (j < (size - 1));
                data[j] = I2C_read_byte(send_ack, false);
            }
            I2C_stop_cond();
            return true;
        }
...
uint8_t I2C_receive_byte(uint8_t address)
{
    ...
    if (I2C_write_byte(address | 0x01, true, false))
    {
        data = I2C_read_byte(false, true); // Tek byte olduğu için NACK ve STOP
        return data;
    }
    I2C_stop_cond();
    return 0;
...
HAL_StatusTypeDef I2C_Write_USB_PD(uint8_t Port, uint16_t Address, uint8_t *DataW, uint16_t Length)
{
	    ...
	    Buffer[1] = DataW[0];
	     if (I2C_transmit(STUSB4500_I2C_ADDR,Buffer,2))
	    	 return HAL_OK;
	     else
	    	 return HAL_ERROR;
}
...
HAL_StatusTypeDef I2C_Read_USB_PD(uint8_t Port, uint16_t Address ,uint8_t *DataR ,uint16_t Length)
{
	uint8_t rxBuffer[1];
	uint8_t reg_addr[1];
	reg_addr[0] = (uint8_t)Address;

	if (I2C_receive(STUSB4500_I2C_ADDR, reg_addr,rxBuffer,1,1))
		return HAL_OK;
	else
		return HAL_ERROR;
}
